...
const PropertyService = {
  list: (params = '') =>
    api.get(`/property/lists${params ? '?' + params : ''}`),
  detail: (slug, id) => api.get(`/property-detail/${slug}/${id}`),
  search: (query = '') =>
    api.get(`/property/lists?search=${encodeURIComponent(query)}`),
  ...
  wishListCreate: propertyId =>
    api.post('/property/wish-list-create', {property_id: propertyId}),
  wishListDelete: propertyId =>
    api.post('/property/wish-list-delete', {property_id: propertyId}),
};
