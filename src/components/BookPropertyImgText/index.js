...
function BookPropertyImgText(props) {
  ...
  const locationStr = [item?.address, item?.city, item?.country].filter(Boolean).join(', ') || 'Location not specified';
  ...
  return (
    <>
      ...
      <View style={props.propertyDetaileContentStyle}>
        ...
        <FacilitiesRules
          facilitiesListData={item?.amenities?.length > 0 ? item.amenities.slice(0, 9).map((a, i) => ({
            id: i,
