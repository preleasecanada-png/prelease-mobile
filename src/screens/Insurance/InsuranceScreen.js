...
const InsuranceScreen = ({ navigation }) => {
  ...
  const fetchInsurance = async () => {
    try {
      ...
      if (res?.data) setPolicies(Array.isArray(res.data) ? res.data : res.data.data || []);
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        <Text style={styles.title} numberOfLines={1}>{item.property?.title || 'Insurance Policy'}</Text>
      ...
      <View style={styles.row}>
        <View style={styles.col}>
          ...
          <Text style={styles.value}>${Number(item.coverage_amount || 0).toLocaleString()}</Text>
        </View>
        <View style={styles.col}>
          <Text style={styles.label}>Premium</Text>
          <Text style={styles.value}>${Number(item.monthly_premium || 0).toLocaleString()}/mo</Text>
        </View>
        <View style={styles.col}>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchInsurance(); }} tintColor={Colors.primary} />}
        ListEmptyComponent={
          <View style={styles.empty}>
            <Icon name="shield" size={48} color="#ccc" />
