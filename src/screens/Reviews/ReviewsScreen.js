...
const ReviewsScreen = ({ navigation }) => {
  ...
  const fetchReviews = async () => {
    try {
      ...
      if (res?.data) setReviews(Array.isArray(res.data) ? res.data : res.data.data || []);
  ...
  const renderStars = (rating) => {
    return Array.from({ length: 5 }, (_, i) => (
      <Icon key={i} name="star" size={14} color={i < rating ? '#ffc107' : '#ddd'} />
    ));
  };
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        <Text style={styles.property} numberOfLines={1}>{item.property?.title || 'Property'}</Text>
