...
import { View, Text, FlatList, TouchableOpacity, ImageBackground, ActivityIndicator, RefreshControl } from 'react-native';
...
const MyRentalPropertyScreen = ({ navigation }) => {
  ...
  const fetchProperties = async () => {
    ...
    } catch (e) { console.error('Failed to load my properties:', e); }
    setLoading(false);
    setRefreshing(false);
  };
    ...
    return (
      ...
      >
        ...
        <View style={{ marginTop: 6 }}>
          <Text style={{ fontSize: 15, fontWeight: '600' }}>{item.title || 'Property'}</Text>
          <Text style={{ fontSize: 13, color: '#666' }}>{[item.address, item.city].filter(Boolean).join(', ')}</Text>
          {item.price && (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={{ alignItems: 'center', marginTop: 60 }}>
                ...
                <Text style={{ fontSize: 16, color: '#999', marginTop: 12 }}>No properties listed yet</Text>
              </View>
            }
          />
        )}
