...
function ChatScreen({navigation}) {
  ...
  const getOtherUser = conv => {
    ...
    if (conv.sender_id === currentUser?.id) {
  ...
  const renderItem = ({item}) => {
    ...
    if (typeof lastMsgObj === 'object' && lastMsgObj) {
      lastMsg =
        typeof lastMsgObj.message === 'string'
          ? lastMsgObj.message
          : String(lastMsgObj.message || '');
        ...
        onLongPress={() => {
          Alert.alert(
            ...
            [
              ...
              {
                text: item.is_pinned ? 'Desepingler' : 'Ã‰pingler en haut',
                onPress: async () => {
                  try {
              ...
              {
                text: unread ? 'Marquer comme lu' : 'Marquer comme non lu',
                onPress: async () => {
                  try {
                    if (unread) {
              ...
              },
              {
                ...
                onPress: () => {
                  Alert.alert(
                    'Confirmation',
                    'Etes-vous sur de vouloir supprimer tous les messages avec cette personne ?',
                    [
  ...
  return (
    <Container
      safeAreaView
      conatinerStyle={{alignItems: 'flex-start', justifyContent: 'flex-start'}}>
      <HeaderMain
        absolute={false}
