    ...
    try {
      if (chatUserId) {
        ...
        const mapped = chatList.map((msg, i) => {
          let msgText = '';
          if (typeof msg.message === 'string') msgText = msg.message;
          else if (typeof msg.text === 'string') msgText = msg.text;
          else if (msg.message && typeof msg.message === 'object') msgText = msg.message.message || JSON.stringify(msg.message);
          else msgText = 'Empty message';
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, backgroundColor: Colors.white }}>
        <Header transparent hasBackBtn title={chatUserName} onBackPress={() => navigation.goBack()} />
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        <GiftedChat
          ...
          renderAvatar={() => null}
          textInputProps={{ style: { flex: 1, padding: 10, fontSize: 16, backgroundColor: '#f0f0f0', borderRadius: 20, margin: 10 } }}
