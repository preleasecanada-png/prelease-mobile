...
function NotificationScreen({ navigation }) {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (res?.data) setNotifications(Array.isArray(res.data) ? res.data : res.data.data || []);
  ...
  const renderItem = ({ item }) => (
    ...
    >
      <View style={nStyles.iconWrap}>
        <Icon
          name={item.type === 'payment' ? 'credit-card' : item.type === 'lease' ? 'file-text' : item.type === 'maintenance' ? 'tool' : 'bell'}
          size={20}
          color={Colors.primary}
        />
      </View>
      <View style={nStyles.content}>
        <Text style={nStyles.title} numberOfLines={1}>{item.title || 'Notification'}</Text>
        <Text style={nStyles.message} numberOfLines={2}>{item.message || item.data?.message || ''}</Text>
        <Text style={nStyles.time}>{item.created_at ? new Date(item.created_at).toLocaleString() : ''}</Text>
      </View>
    </TouchableOpacity>
  );

  if (loading) return <View style={nStyles.center}><ActivityIndicator size="large" color={Colors.primary} /></View>;
...
const nStyles = StyleSheet.create({
  ...
  list: { padding: 16 },
  card: { flexDirection: 'row', padding: 14, borderRadius: 12, borderWidth: 1, borderColor: '#eee', marginBottom: 10, backgroundColor: '#fff' },
  unread: { backgroundColor: '#fef2f2', borderColor: '#fecdd3' },
  iconWrap: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#f0f0f0', justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  content: { flex: 1 },
