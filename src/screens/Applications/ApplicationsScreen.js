      ...
      <View style={styles.cardRow}>
        <View style={styles.cardCol}>
          ...
          <Text style={styles.cardValue}>
            {item.desired_move_in ? new Date(item.desired_move_in).toLocaleDateString() : '—'}
          </Text>
        </View>
        <View style={styles.cardCol}>
          <Text style={styles.cardLabel}>Duration</Text>
          <Text style={styles.cardValue}>{item.desired_lease_duration?.replace('_', ' ')}</Text>
        </View>
        <View style={styles.cardCol}>
          <Text style={styles.cardLabel}>Income</Text>
          <Text style={styles.cardValue}>${Number(item.monthly_income || 0).toLocaleString()}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
...
const styles = StyleSheet.create({
  ...
  list: { padding: 16 },
  card: { backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#eee', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
