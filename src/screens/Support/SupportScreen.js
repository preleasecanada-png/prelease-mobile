...
const priorityColors = { low: '#198754', medium: '#ffc107', high: '#fd7e14', urgent: '#dc3545' };
const statusColors = { open: '#0d6efd', in_progress: '#0dcaf0', resolved: '#198754', closed: '#6c757d' };
...
const SupportScreen = ({ navigation }) => {
  ...
  const fetchTickets = async () => {
    try {
      ...
      if (res?.data) setTickets(Array.isArray(res.data) ? res.data : res.data.data || []);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await SupportService.create({ subject, message, priority: 'medium' });
      if (res?.data) {
    ...
    <View style={styles.container}>
      ...
      {showForm && (
        <View style={styles.form}>
          ...
          <TextInput style={[styles.input, { height: 80, textAlignVertical: 'top' }]} placeholder="Describe your issue..." value={message} onChangeText={setMessage} multiline />
...
const styles = StyleSheet.create({
  ...
  message: { fontSize: 13, color: '#666', marginBottom: 8 },
